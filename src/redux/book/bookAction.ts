...
export const createBook = (input: createBookProps) => async (dispatch: AppDispatch) => {
...
export const searchByTitle = (keyword: string) => async (dispatch: AppDispatch) => {
...
export const searchByGenre = (name: string[]) => async (dispatch: AppDispatch) => {
  dispatch({ type: BOOK_SEARCH_REQUEST });
  try {
    const response = await axiosInstance.get(`${urls.booksUrl}`, {
      params: { genres: name },
